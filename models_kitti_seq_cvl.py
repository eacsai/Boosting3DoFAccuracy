...
from VGG import VGGUnet, Encoder, Decoder, Decoder2, Decoder4, VGG16
...
class Model(nn.Module):
    def __init__(self, args, device, feature_win=32):  # device='cuda:0',
        ...
        self.meters_per_pixel = boost_utils.get_meter_per_pixel()
        ...
        self.FuseNet = TransformerFusion(seq=self.args.sequence, n_embd=out_c, n_head=2, n_layers=2)
    ...
    def corr(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
        ...
        grd_feat_proj, _, u, mask = self.project_grd_to_map(
            grd_feature_l, None, shift_u, shift_v, heading, left_camera_k, A, ori_grdH, ori_grdW) # [B,S,E,C,H,W]
        ...
        corr_maps = []        
        self.meter_per_pixel = boost_utils.get_meter_per_pixel() * boost_utils.SatMap_original_sidelength / A
        
        crop_H = int(A - self.args.shift_range_lat * 3 / self.meter_per_pixel)
        crop_W = int(A - self.args.shift_range_lon * 3 / self.meter_per_pixel)
        g2s_feat = TF.center_crop(grd_feature, [crop_H, crop_W])
        g2s_feat = F.normalize(g2s_feat.reshape(B, -1)).reshape(B, -1, crop_H, crop_W)

        s_feat = sat_feature.reshape(1, -1, A, A)  # [B, C, H, W]->[1, B*C, H, W]
        corr = F.conv2d(s_feat, g2s_feat, groups=B)[0]  # [B, H, W]

        denominator = F.avg_pool2d(sat_feature.pow(2), (crop_H, crop_W), stride=1, divisor_override=1)  # [B, 4W]
        ...
        pred_u = (max_index % corr_W - corr_W / 2) * self.meter_per_pixel  # / self.args.shift_range_lon
        pred_v = -(max_index // corr_W - corr_H / 2) * self.meter_per_pixel  # / self.args.shift_range_lat
        ...
        # sat_features_to_RGB(sat_feature, grd_feature)
    ...
    def triplet_loss(self, corr_maps, gt_shift_u, gt_shift_v, gt_heading):
        ...
        w = torch.round(corr_W / 2 - 0.5 + gt_delta_x_rot / self.meter_per_pixel)
        h = torch.round(corr_H / 2 - 0.5 + gt_delta_y_rot / self.meter_per_pixel)
    ...
    def CVattn_corr(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
        ...
        for level in range(len(sat_feat_list)):
            ...
            g2s_feat = TF.resize(grd_feat, [crop_H, crop_W])
            g2s_feat = F.normalize(g2s_feat.reshape(B, -1)).reshape(B, -1, crop_H, crop_W)

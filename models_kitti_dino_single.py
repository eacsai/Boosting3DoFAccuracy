...
def vis_corr(corr):
    corr = corr.clone().cpu()
    # 将 corr 转换为 NumPy 数组
    corr_np = corr.detach().numpy()
    # 获取图像的高度和宽度
    H, W = corr_np.shape[1:]
    # 创建图像并将原点设置在中心
    fig, ax = plt.subplots()
    cax = ax.imshow(corr_np[0], cmap='hot', interpolation='nearest', extent=(-W//2, W//2, -H//2, H//2))
    fig.colorbar(cax)
    # 设置标题和坐标轴标签
    ax.set_title('Correlation Map with Centered Origin')
...
class Model(nn.Module):
    ...
    def corr(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
        ...
        meter_per_pixel = boost_utils.get_meter_per_pixel() * 512 / 128

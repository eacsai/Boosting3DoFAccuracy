...
def get_BEV_kitti(front_img, fov, pitch, scale, out_size, device, heading = 0, Tx = 0, Ty = 0): 
    ...
    # Rotation and translation matrix H2
    gamma = heading * torch.pi / 180  # Convert gamma to radians
    uc = out_size / 2
    vc = out_size / 2
    cos_gamma = np.cos(gamma)
    sin_gamma = np.sin(gamma)
    H2 = torch.tensor([
        [cos_gamma, -sin_gamma, uc*(1-cos_gamma) + vc*sin_gamma],
        [sin_gamma, cos_gamma, uc*(1-cos_gamma) - vc*sin_gamma],
        [0, 0, 1]
    ], dtype=torch.float32).to(device)
    
    T_translate_back = torch.tensor([
        [1, 0, (out_size * sin_gamma / 2) + Tx],
        [0, 1, (-out_size * cos_gamma / 2) + Ty],
        [0, 0, 1]
    ], dtype=torch.float32).to(device)
    
    Homo = torch.matmul(T_translate_back, torch.matmul(H2, torch.matmul(H1, H))) 
